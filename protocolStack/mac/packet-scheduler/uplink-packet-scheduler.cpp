...
{
#ifdef SCHEDULER_DEBUG
	std::cout << " ---- UL RBs Allocation (Uplink SChed)";
#endif
...
#ifdef SCHEDULER_DEBUG
  std::cout << ",UL available RBs " << nbOfRBs << ", users " << users->size () << std::endl;
...
//HB
double UplinkPacketScheduler::CalculatePower(int nbRBs){
	double P0 = -57.0;
	double Pmax = 23;
	double power;

	power = P0 + 10 * log10(nbRBs);
	return power;
