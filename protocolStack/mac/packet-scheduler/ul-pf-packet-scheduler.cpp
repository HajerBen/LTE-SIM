...
double UlPFScheduler::ComputeSchedulingMetric(RadioBearer *bearer,
		double spectralEfficiency, int subChannel) {
	double metric;
	return metric;
}

double UlPFScheduler::ComputeSchedulingMetric(UserToSchedule* user,
		int subChannel) {
	double metric;
	return metric;
}
...
void UlPFScheduler::RBsAllocation() {
	...
	for (int j = 0; j < users->size(); j++) {
		...
		for (RrcEntity::RadioBearersContainer::iterator it =
			...
			for (int i = 0; i < nbOfRBs; i++) {
				metrics[i][j] = ComputeSchedulingMetric(scheduledUser, b, i);
