...
void SchedulerTest::RBsAllocation() {
		...
		requiredPRBsH2H[j] = (floor)(
				scheduledUser->m_dataToTransmit
						/ (GetMacEntity()->GetAmcModule()->GetTBSizeFromMCS(mcs,
								1) / 8));
		requiredPRBs[scheduledUser->m_userToSchedule->GetIDNetworkNode()] = (floor)(
				scheduledUser->m_dataToTransmit
						/ (GetMacEntity()->GetAmcModule()->GetTBSizeFromMCS(mcs,
								1) / 8));
#ifdef SCHEDULER_DEBUG1
	...
	for (int i =0; i< users->size(); i++){
		scheduledUser =users->at(i);
		std::cout << "requiredPRBs["<<scheduledUser->m_userToSchedule->GetIDNetworkNode()<< "]="
				<<requiredPRBs[scheduledUser->m_userToSchedule->GetIDNetworkNode()] << endl;
	}
					...
					while ((ContinueRight || ContinueLeft) && availableRBs > 0) {
						...
						if (right < nbOfRBs && !Allocated[right]) {
							...
							if (ContinueRight) {
								...
								users->at(selectedUser)->m_listOfAllocatedRBs.push_back(
										right);
								MatriceOfAllocation[right] = selectedUser;
								right++;
								availableRBs--;
								//std::cout << "right allocation" << std::endl;
							}
						...
						if (left >= 0 && !Allocated[left]) {
							...
							if (ContinueLeft) {
								Allocated[left] = true;
								MatriceOfAllocation[left] = selectedUser;
								users->at(selectedUser)->m_listOfAllocatedRBs.push_back(
										left);
								left--;
								availableRBs--;
