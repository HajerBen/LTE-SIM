...
#ifdef Allocation
		std::cout << " selected User " << selectedUser << " delay "
				<< lowestDelay << std::endl;
#endif
...
//sort channels
			...
			for (int i = 0; i < ChannelsSorted.size() && Verify; i++) {
				VerifyPF = true;
				verifyPower = false;
				verifyPower = VerifyPower(nbRB[i], requiredPRBsM2M[selectedUser],
						scheduledUser);
