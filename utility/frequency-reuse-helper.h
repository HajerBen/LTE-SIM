...
static std::vector<BandwidthManager*> RunFrequencyReuseTechniques(int nodes,
		int cluster, double bandwidth) {
	...
	if (FrameManager::Init()->GetFrameStructure()
		...
		if (bandwidth == 1.4) {
			...
			int operatibeSubBands = 6;
			int counter = 0;

			for (int i = 0; i < nodes; i++) {
				int offset = counter * operatibeSubBands;

				BandwidthManager *s = new BandwidthManager(bandwidth, bandwidth,
						offset, offset);
