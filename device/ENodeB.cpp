...
ENodeB::ENodeB(int idElement, Cell *cell) {
	...
	CartesianCoordinates *position = new CartesianCoordinates(
...
std::vector<double> ENodeB::UserEquipmentRecord::GetUplinkChannelStatusIndicator(
		void) const {
...
void ENodeB::SetULScheduler(ULSchedulerType type) {
	...
	case ENodeB::ULScheduler_TYPE_MYSCHEDULER:
		...
		mac->SetUplinkPacketScheduler(scheduler);
		break;
	case ENodeB::ULScheduler_TYPE_Recursive_Maximum_Expansion:
		scheduler = new RecursiveMaximumExpansion();
		scheduler->SetMacEntity(mac);
		mac->SetUplinkPacketScheduler(scheduler);
		break;
	...
	case ENodeB::LIOUMPAS_V2_UPLINK_SCHEDULER:
		...
		mac->SetUplinkPacketScheduler(scheduler);
		break;
	case ENodeB::ULScheduler_TYPE_PROPORTIONAL_FAIR:
		scheduler = new UlPFScheduler();
		scheduler->SetMacEntity(mac);
		mac->SetUplinkPacketScheduler(scheduler);
		break;
